(function () {
  let _onload = function () {
      ...
      const render = function () {
        ...
        for (let j = -1.57; j <= 1.57; j += 0.2) {
          ...
          for (i = 0; i < 6.28; i += 0.2) {
            ...
            let x = ox * cp + X; // final 3D x coordinate
            let y = -ox * sA * sp + oy * cA; // final 3D y
            ...
            const L = 0.7 * (cA * ct * sp - sA * st + cA * st - ct * sA * sp);
            if (L > 0) {
    ...
    window.anim2 = function () {
      if (tmr2 === undefined) {
        tmr2 = setInterval(sphere, 50, 0.07, 0.03);
      } else {
        clearInterval(tmr2);
        tmr2 = undefined;
      }
    ...
    // sun.render();
    earth = createSphere({ R1: 50, R2: 0, K1: 2500, K2: 3000, B: 0, X: 300 });
    earth.render();
  };
