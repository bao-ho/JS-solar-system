  ...
  let _onload = function () {
    ...
    const createSphere = ({ R1, R2, K1, K2, A0, B, X }) => {
      ...
      const render = function () {
        ...
        for (let j = -1.57; j <= 1.57; j += 0.2) {
          ...
          for (i = 0; i < 6.28; i += 0.2) {
            ...
            let x = x1 * cd - z1 * sd;
            let y = y1;
            let ooz = 1 / (K2 + (x1 * sd + z1 * cd)); // one over z
            ...
            const L = 0.7 * (st - ct * cp * sd - ct * sp * cd);
            if (L > 0) {
      ...
      const api = {
        ...
        doublyRotateAroundYAxis(deltaC, deltaD) {
          C += deltaC;
          D += deltaD;
          render();
        },
    ...
    window.anim2 = function () {
      if (tmr2 === undefined) {
        tmr2 = setInterval(earth.doublyRotateAroundYAxis, 50, 0.01, 0.01);
      } else {
        clearInterval(tmr2);
        tmr2 = undefined;
      }
    ...
    // sun.render();
    earth = createSphere({ R1: 50, R2: 0, K1: 5000, K2: 6000, B: 0, X: 400 });
    earth.render();
  };
